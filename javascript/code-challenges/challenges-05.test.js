...
const toLastNames = people => {
  ...
  // let array = [];
  // people.map((item) => {
  //   return array.push(`${item.firstName} ${item.lastName}`);
  // });
  // return array;


 
  let array= people.map((item)=>{
...
const addValues = (arr) => {
  let sumnum = arr.reduce((out, item) => {
    return out + item;
  }, 0);
  return sumnum;
};
...
const countNumberOfElements = (arr) => {
  let sumnum = arr.reduce((out, item, i) => {
    return i + 1;
  }, 0);
  return sumnum;
};
