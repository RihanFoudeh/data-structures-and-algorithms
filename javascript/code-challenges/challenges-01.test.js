...
const addExclamation = (arr1) => {
  const arrTeststr = [];
  arr1.forEach((value) => [
    arrTeststr.push(value =value+'!' )
  ]);
  return arrTeststr;
};
...
const createList = (availableItems) => {
 const availablefruts =[];
 availableItems.forEach((value) =>{
 if (value.available===true){

availablefruts.push(value.name);
 }
 });

 return availablefruts;
